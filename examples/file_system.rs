...
//! - **GVar projections**: extracting `FileHandle` from `Result<(FileHandle, String), FsErr>`
//!   via `OpOk` then `OpFst`
//! - **Composed bridges**: `ResultBridge<TupleBridge<Opaque, Transparent>, Transparent>`
//! - **Variable tracking**: `uses = [handle]`
...
impl LockstepModel for FsLockstep {
    ...
    fn gen_action(_state: &MockFs, env: &TypedEnv) -> BoxedStrategy<Box<dyn AnyAction>> {
        ...
        // Collect open-result variables for Write/Close generation
...
fn main() {
    println!("Run with `cargo test --example file_system`");
}
